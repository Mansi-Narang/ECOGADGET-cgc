...
app.post("/signup", async (req, res) => {
  ...
  if (existingUser) {
    return res.json({ msg: "User already Exists, Kindly Log In", sucess: false });
  }
...
app.post("/login", async (req, res) => {
  ...
  if (!user) {
    return res.json({
      msg: "User doesn't exists", success: false
    });
  }
...
app.post("/sell/device", async (req, res) => {
  ...
  if (!deviceType || !brand || !model || !condition || !description || !askingPrice) {
    return res.json({
      msg: "Incomplete Credentials",
      success: false
    });
  }
  const newSellDevice = new sellDeviceModel({ deviceType, brand, model, condition, description, askingPrice });

  await newSellDevice.save();

  return res.json({
    msg: "Device is queued for examination", success: true
  });
});


app.post("/rent/device", async (req, res) => {
  const { deviceName, deviceType, deviceDescription, dailyRate, location, availableFrom, availableTo } = req.body;

  if (!deviceName || !deviceType || deviceDescription || dailyRate || location || availableFrom || availableTo) {
    return res.json({
      msg: "Incomplete Credentials",
      success: false
    });
  }
  const newRentDevice = new rentDeviceModel({ deviceName, deviceType, deviceDescription, dailyRate, location, availableFrom, availableTo });

  await newRentDevice.save();

  return res.json({
    msg: "Your device has been successfully listed for rent. We'll notify you as soon as someone is interested in renting your device. ", success: true
  });
});
...
app.post("/repair", async(req, res) => {
  ...
  return res.json({ msg: "Our technicians will reach to you out soon!", success: true });
});
...
app.post("/repair/technician", async (req, res) => {
  ...
  return res.json({ msg: "We are reviewing your application...", success: true });
});
